      ...
      for (const pokemon of responseResults) {
        ...
        try {
          const responseUser = await axios.get(
            `${POKEMON_USER_URL}pokemon/score/all`,
            {
              headers: {
          ...
          if (responseUser.data?.data) {
            const responseResults = responseUser.data?.data || [];

            responseResults.forEach((item: any) => {
              let indexTarget = pokeList.findIndex(
                (el: any) => el.id == item.pokemon_id
              );
              pokeList[indexTarget] = {
                ...pokeList[indexTarget],
                score: item.score,
              };
            });
